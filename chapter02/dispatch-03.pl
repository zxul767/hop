...
sub read_config {
    ...
    while (<$file>) {
        ...
        my $action =
          $dispatch_table->{$directive} || $dispatch_table->{_DEFAULT_};
        $action->(
            $directive, $rest, $dispatch_table, $user_context, $filename
        );
    }
    return 1;
}
...
sub define_config_directive {
    ...
    if ( not defined $definition ) {
        warn
          "Couldn't compile definition for '$new_directive': $@; skipping.\n";
        return;
    }
...
sub no_such_directive {
    ...
    warn
      "Unrecognized directive $directive at line $. of $filename; ignoring.\n";
}
